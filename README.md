...
- 🧠 **Launcher memory** — on-device prediction from time of day, weekday rhythm, app-to-app sequences, and context triggers (Bluetooth, headphones, charging, joining/leaving Wi-Fi, losing/regaining signal, starting to move — via the ultra-low-power significant-motion sensor, never the raw accelerometer). Optionally it can tell home from office by Wi-Fi *network*: the name is stored only as a salted hash that nothing outside the app can reproduce (needs precise location). It also learns from your corrections: swipe the suggestions away and open something else within seconds, and the wrongly-shown apps take a lasting, context-aware penalty while the app you reached for gets extra credit — swiping just to play changes nothing. The whole learning history can be exported as a JSON file from settings for your own analysis.
- 📌 **Drop-to-pin slot** — drag any app toward the circle that appears above the trio and drop it: it stays pinned there for a few hours (duration, manual pick, and a rotating multi-app mode in settings). The slot leans toward your finger as the drag approaches, and slides in past an open drawer so it is always reachable.
- ✋ **Lift anything** — long-press any app anywhere (suggestions, search results, the spotlight, the pin slot, a drawer) to pick it up: let go in place for its menu, or carry it. Nearing a screen edge opens that side's drawer to receive it; carrying a drawer or pinned app to the top or bottom of the screen reveals a **remove** band — drop there to take it out of the drawer or unpin it.
...
- 🧭 **Transparent brain** — *Settings → How suggestions work* shows a live terminal-style report of the engine: its signal weights, how much data it keeps (and the database's size on disk, also available as a status-line field), the current context, and the live top-10 ranking. It also **grades itself**: a live hit-rate (how often the trio contained what you actually launched, plus a confidence-reliability table) and a **backtest** that replays your whole history walk-forward and scores the engine against frequency-only and recency-only baselines. Long-press any app to **boost** it in the suggestions; long-press a home-screen element to jump straight to its settings.
- 🎵 **Now-playing controls** — while music plays (or pauses), a compact terminal-style player line appears under the clock: `♪ artist — title` with previous/play-pause/next, tap the title to open the app. Since the row already covers it, the playing app politely steps out of the suggestion trio. Uses the same notification access as badges; can be turned off.
...
- 📅 **Agenda stream** — your next calendar events as quiet terminal lines under the status line: the next one leads in the accent color with a live countdown, later days get a `┄ tomorrow` separator, and the whole stream scrolls through the days ahead (3/7/14, configurable). Tap an event to unfold it in place — the location is one tap from navigation — or long-press to open it in your calendar app. Choose which calendars appear, or flip on *Show only on gesture* and bind **Agenda** to an edge swipe to summon it on demand. A same-day birthday or holiday never gets lost behind a later timed event either — pick how the two share the spotlight (highlight both, give all-day its own marker, or let whichever is soonest win). Read-only; events never leave the phone.
- 👆 **Double-tap to lock** the screen (tiny accessibility service, never reads screen content)
- 📲 **One page, zero friction** — swipe up for all apps (keyboard opens with it), swipe down for notifications (two fingers for quick settings), long-press for settings
